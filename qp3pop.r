...
mypops = c('Europe','Israel','Caucasus','Middle_east','India','Baltics','Latvia','Croatia')
...
for (x in mypops) {
    ...
    sapply(paste0("f3_results_", x), file.create)
    write.table(rrs, file = paste0("f3_results_", x), sep = "\t", row.names=FALSE, append=TRUE)
}

...
#SBATCH -o treemixPruned_freq_%A.log	## name a file to save log of your job
#SBATCH -e treemixPruned_freq_%A.error	## save error message if any	
...
GENO=$HAP/ExpandedGoldenJackals_haplo_filtered_pruned_Pop.geno
IND_POP=$HAP/ExpandedGoldenJackals_haplo_filtered_pruned_Pop.ind
# Column1(Sample+id) Col2(Sex, Normally U), Col3(Pop)
OUT='ExpandedGJ_pruned.freq'
...
grep -v "0,0" $WOK/$OUT | gzip -c > $WOK/ExpandedGJ_pruned_noMiss.freq.gz
...
#treemix -i ExpandedGJ_fox_noMiss.freq.gz -m $mig -o Pops.m"$mig"_withBase -g Pops.test_0.vertices.gz Pops.test_0.edges.gz -bootstrap -k 500 -n_warn 12 > treemix_Wbase"$mig"_log

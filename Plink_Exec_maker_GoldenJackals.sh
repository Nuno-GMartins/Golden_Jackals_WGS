...
haploToPlink $HPC/$BG.gz $HPC/"$na"_haplo
...
cp $HPC/"$na"_haplo.tped $HPC/"$na"_haplo.old.tped

cat $HPC/"$na"_haplo.old.tped | sed 's/N N/0 0/g' > $HPC/"$na"_haplo.tped

plink2 --tfile $HPC/"$na"_haplo --allow-extra-chr --make-bed --out $HPC/"$na"_haplo --chr-set 38
...
awk '{print $1}' $HPC/"$na"_haplo.fam > $HPC/listA #list the names of the fam file (ind0-n)
...
cp $HPC/"$na"_haplo.fam $HPC/"$na"_haplo.old.fam
...
' $HPC/listA $HPC/listB $HPC/"$na"_haplo.old.fam > $HPC/"$na"_haplo.fam

sed -i 's/-9$/1/' $HPC/"$na"_haplo.fam
...
awk '{print $2,$5,$6}' $HPC/"$na"_haplo.bim > $HPC/variant_ID.txt
...
plink2 --bfile $HPC/"$na"_haplo --allow-extra-chr --chr-set 38 --exclude ID --make-bed --out $HPC/"$na"_haplo_filtered
...
/projects/mjolnir1/people/jbf527/ref/add_sample_to_eig.sh "$na"_haplo_filtered fox meta.txt _ $HPC

module unload plink/2.0.0

module load plink/1.9.0
plink --bfile "$na"_haplo_filtered --indep-pairwise 50 5 0.5 --out "$na" --allow-extra-chr --chr-set 38

module unload plink/1.9.0

module load plink/2.0.0
plink2 --bfile "$na"_haplo_filtered --allow-extra-chr --chr-set 38 --exclude "$na".prune.out --make-bed --out "$na"_haplo_filtered_pruned

module unload plink/2.0.0

module load plink/1.9.0
plink --bfile "$na"_haplo_filtered_pruned --allow-extra-chr --chr-set 38 --maf 0.1 --make-bed --out "$na"_haplo_filtered_pruned-filtercount
